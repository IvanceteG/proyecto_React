...
export default function App() {
  ...
  const handleEdit = async (datos) => {
    try {
      const actualizado = await alumnosService.update(alumnoEditando._id, datos);
      setAlumnos(alumnos.map((a) => (a._id === actualizado._id ? actualizado : a)));
      setFormularioAbierto(false);
      ...
      <main className="main">
        ...
        {error && (
          <div className="error-box">
            <p>⚠️ {error}</p>
            <code>npm run dev (backend_alumnos)</code>
          </div>
        )}
          ...
          <div className="alumnos-grid">
            {alumnosFiltrados.map((alumno) => (
              <Alumno
                ...
                onDelete={() => handleDelete(alumno._id)}
              >
